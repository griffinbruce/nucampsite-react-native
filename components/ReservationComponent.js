...
class Reservation extends Component {
    ...
    async obtainNoticationPermission() {
        ...
        if (permission.status !== 'granted') {
            ...
            if (permission.status !== 'granted') {
                Alert.alert('Permission not granted to show notifictations');
            }
            return permission;
        }
        return permission;
    ...
    async presentLocalNotification(date) {
        const permission = await this.obtainNoticationPermission()
        if (permission.status === 'granted') {
    ...
    render() {
        return(
            //Added additional scrollview to fix ios formatting issue. this does change the look on android a bit, but it at least makes the ios version readable
            <ScrollView>
                <Animatable.View animation='zoomIn' duration={2000} delay={1000}>
                    ...
                    <View style={styles.formRow}>
                        ...
                        <DatePicker
                            style={{flex: 2, marginRight: 20, marginBottom: 10}}
                            date={this.state.date}
                            format='YYYY-MM-DD'
                            mode='date'
